...
对于**多分类问题**，用 C 表示种类个数，则神经网络输出层，也就是第 L 层的单元数量 $n^{[L]} = C$。每个神经元的输出依次对应属于该类的概率，即 $P(y = c|x), c = 0, 1, .., C-1$。有一种逻辑回归的一般形式，叫做 **Softmax 回归**，可以处理多分类问题。
...
多分类的 Softmax 回归模型与二分类的逻辑回归模型只有输出层上有一点区别。经过不太一样的推导过程，仍有

$$dZ^{[L]} = A^{[L]} - Y$$

反向传播过程的其他步骤也和逻辑回归的一致。
